REM - Top Up Tap Cash Automation test
Dim dt_TCID, dt_TestScenarioDesc, dt_ScenarioDesc, dt_ExpectedResult
Dim dt_NoTapCash, dt_Nominal, dt_SaveName, dt_PinTrx
Dim dt_Username, dt_Password

REM -------------- Call Function
Call spLoadLibrary()
Call spInitiateData("KeagenanMobile_Lib_Report.xlsx", "SMAG-0040_TopUp_TapCash.xlsx", "SMAG-0040")
Call spGetDatatable()
...
Call Open_MAgen()
Call Goto_ToUp_TapCash()

If dt_TCID = "MAG0040-003" Then
'    Call Choice_Fav()
	Msgbox("Pilih Vaforit")
Else
	Call Input_Baru(dt_TCID, dt_NoTapCash)
End If

If dt_TCID = "MAG0040-009" Then
	Call Input_Nominal_Lain(dt_Nominal, dt_SaveName)
Else
	Call Pilih_Nominal()
End If

If dt_TCID = "MAG0040-009" Then
	Call Verify_Fail_MaxMinNominal()
Else
	Call Input_Pass(dt_TCID, dt_PinTrx)
End If

If dt_TCID = "MAG0040-001" or dt_TCID = "MAG0040-003"  Then
	Call Verify_Success()
ElseIf dt_TCID = "MAG0040-002" Then
	Call Verify_Success_Save()
ElseIf dt_TCID = "MAG0040-006" Then
	Call Verify_Fail_Empty_Pass()
Else
	If dt_TCID <> "MAG0040-009" Then
		Call Verify_Fail()
	End If
...
Sub spLoadLibrary()
	...
	REM ---- Top Up Tap Cash Keagenan Mobile Library
	LoadFunctionLibrary (LibPathKeagenanMobile & "Lib_KeagenanMobile_TopUpTapCash.qfl")
	Call RepositoriesCollection.Add(LibRepo & "RP_KeagenanMobile_TopUpTapCash.tsr")
End Sub
...
Sub spGetDatatable()
	...
	REM --------- Top Up TapCash
	dt_NoTapCash	= DataTable.Value("NO_TAPCASH", dtLocalSheet)
